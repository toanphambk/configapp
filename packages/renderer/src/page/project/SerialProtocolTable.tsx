...
import {ProtocolEnum, SerialProtocolEnum} from '../../../../main/src/enums/Protocols.enum';
...
const createModalFields: ModalFormField<Protocol>[] = [
  {
    key: 'type',
    label: 'Protocol',
    inputType: 'select',
    rules: [{required: true, message: 'Please select a protocol type!'}],
    selectOptions: getEnumOptions(SerialProtocolEnum),
  },
];
...
const SerialProtocolTable: React.FC = () => {
  ...
  const {mutateAsync: create} = useCreateProtocol({}, []);

  const {mutateAsync: update} = useUpdateProtocol({}, []);
...
export default SerialProtocolTable;
